...
import { joinEvent, leaveEvent, isJoinedEvent } from 'utils/api';
...
const AuthEventButton: React.FC<{ joined: boolean, id: string }> = ({ joined, id }) => {
  ...
  return(
    <div>
      <Button version="secondary" onClick={handleButtonAction}> {buttonText} </Button>
    </div>
  )
}
...
const AuthButton: React.FC<{ id: string }> = ({ id }) => {
  ...
  return (
    <div>
      {isJoined !== undefined && (
        <AuthEventButton joined={isJoined} id={id} />
      )}
    </div>
  );
};
...
const DefaultButton = () => {
  ...
  return (
    <div>
      <Button version="secondary" onClick={login}> Bli med! </Button>
    </div>
  );
};
